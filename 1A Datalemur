...
SELECT *
  ...
  FROM filed_taxes
  WHERE UPPER(product) LIKE '%TURBOTAX%'
)
...
SELECT user_id
  ...
  , LAG(yr, 2) OVER(PARTITION BY user_id ORDER BY yr) AS prevprev_yr
...
, loyaltyTest AS (
SELECT *
  , CASE WHEN yr - prev_yr = 1 
      And prev_yr - prevprev_yr = 1
    THEN 1
    ELSE NULL 
    END AS loyal
  FROM prevs
...
SELECT searches
  , num_users
  , SUM(num_users) OVER(ORDER BY searches) AS running 
...
SELECT ROUND(AVG(searches), 1) AS median
  ...
  WHERE running >= total / 2.0 
    AND running - num_users <= total /2.0
...
WITH enhanced AS (
SELECT * 
  , CASE WHEN event_type IN ('sign_in', 'like', 'comment')
      THEN 1 
      ELSE NULL
      END AS active
...
SELECT 
    curr.user_id
  , curr.month
  , curr.ttl_acts AS active_curr
  , prev.month AS prev_mo
  , prev.ttl_acts AS active_prev
  , CASE WHEN curr.ttl_acts > 0 and prev.ttl_acts > 0
    THEN 1 
    ELSE NULL 
    END AS active
  FROM agg AS curr 
  LEFT JOIN agg AS prev
...
SELECT 
    ...
    , ROUND(((curr.spend - prev.spend)/prev.spend)*100, 2) AS yoy_rate
  FROM agg as curr
  LEFT JOIN agg AS prev 
    ON curr.product_id = prev.product_id
    AND curr.year - 1 = prev.year
  ORDER BY curr.product_id ASC,  curr.year ASC 
;
...
WITH Agg AS (
SELECT customer_id
  , COUNT (DISTINCT p.product_category) AS ttl_cats
  FROM customer_contracts AS c 
  JOIN products AS p 
    ON c.product_id = p.product_id
